...
public class UkMakerSpaces {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Dataset<Row> makerSpace = session.read().option("header", "true").csv(base_datadir+"in/uk-makerspaces-identifiable-data.csv");

        Dataset<Row> postCode = session.read().option("header", "true").csv(base_datadir+"in/uk-postcode.csv")
